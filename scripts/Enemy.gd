...
@export var experience_value: float = 100.0
...
func _ready():
	...
	scale = Vector2(0.5, 0.5)  # 将敌人的大小减小到原来的50%
	
	# 创建敌人形状
	var enemy_shape = Polygon2D.new()
	enemy_shape.polygon = [
		Vector2(0, -15),  # 顶部
		Vector2(-8, 8),  # 左下
		Vector2(8, 8),  # 右下
	]
	enemy_shape.color = Color(1, 0.2, 0.2)  # 设置颜色
	add_child(enemy_shape)  # 将敌人形状添加到场景中
...
func _on_body_entered(body):
	if body.is_in_group("bullet"):
		take_damage(body.damage)  # 假设子弹有 damage 属性
...
func take_damage(amount: float):
	...
	if current_health <= 0:
		...
		if player:
			player.gain_exp(experience_value)  # 掉落经验值
