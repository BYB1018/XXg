...
var base_upgrades = {
	...
	"bounce": {
		"name": "弹射射击",
		"description": "子弹可以弹射到其他敌人",
		"effect": "_apply_bounce"
	},
	"homing": {
		"name": "追踪射击",
...
func show_upgrades():
	...
	for combo_id in combo_upgrades.keys():
		...
		for req in requirements:
			if req not in obtained_upgrades:
				can_combo = false
				break
		if can_combo and combo_id not in obtained_upgrades:
			available_upgrades.append(combo_id)
	
	# 添加基础升级
	for upgrade_id in base_upgrades.keys():
		if upgrade_id not in obtained_upgrades:
...
func _on_upgrade_selected(upgrade: String):
	...
	call(effect_name, get_tree().get_first_node_in_group("player"))
	
	# 记录已获得的升级
	obtained_upgrades.append(upgrade)
